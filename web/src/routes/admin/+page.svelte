<script lang="ts">
	...
	const filters = ['', 'new', 'preparing', 'ready', 'out_for_delivery', 'completed', 'cancelled'];
...
<nav class="tabs" aria-label="Filter orders">
	{#each filters as f (f)}
		<a href={f ? `?status=${f}` : '/admin'} aria-current={data.status === f ? 'page' : undefined}>
			{f ? labels[f] : 'All'}
		</a>
	{/each}
</nav>
...
{#if !data.orders.length}
	<p class="card empty">No orders here yet. New ones show up automatically.</p>
{:else}
	<ul class="orders">
