...
- `mobile/` Expo (React Native) customer app: `App.tsx` (boot, menu, cart bar), `Cart.tsx`, `SignIn.tsx`, `Account.tsx`, `Onboarding.tsx`; building blocks in `ui.tsx` (`Button`, `IconButton`, `Stepper`, `Icon`) and `Rolling.tsx` (odometer digits for money and counts). Fonts are tokens in `theme.ts` (`f.*`: Mona Sans, Geist Mono for order numbers); set `fontFamily` on every text style, not `fontWeight`. Don't use `tabular-nums` with Mona Sans (its tabular digits look typewriter). Motion uses `ms()`/`native` from `motion.ts`. Token in SecureStore via `store.ts`. Customer sign-in is optional (text-message code, `api/customers.go`). Shares types, VAT maths, error messages and `Intl` formatters with web through `web/src/lib/shared.ts` (framework-free; `web/src/lib/api.ts` re-exports it). Keep `$env`/`$lib` imports out of `shared.ts` or the mobile bundle breaks. Add packages with `npx expo install`, not `pnpm add`. Build locally (`npx expo run:ios|android`), not EAS.
...
- Adding a column to `orders` breaks any running API built before it (`select *` scans a fixed column list): restart the API after migrating.
